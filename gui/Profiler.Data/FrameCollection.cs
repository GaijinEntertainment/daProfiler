...
namespace Profiler.Data
{
	...
	public class FrameGroup
	{
		...
		public SamplingDescriptionPack SamplingBoard { get; set; }
		public CallstackPack Callstacks { get; set; }
			...
			if (Cores != null && !IsCoreDataGenerated)
			{
				foreach (KeyValuePair<UInt64, Synchronization> pair in Synchronization.SyncMap)
				{
					...
					foreach (SyncInterval interval in pair.Value)
					{
						...
						Entry entry = new Entry(eventDesc, interval.Start, interval.Finish, -1);
		...
		public void AddCallStackPack(CallstackPack pack)
		{
			...
			for (int i = 0; i < Threads.Count; ++i)
			{
				...
				if (Callstacks.CallstackMap.TryGetValue(Board.Threads[i].ThreadID, out callstacks))
				{
				...
				foreach (FiberSyncInterval fiberSync in frame.FiberSync)
				{
					...
					for (int i = 0; i <= lastIndex; i++)
					{
						...
						if (entry.Intersect(border))
						{
							...
							entries.Add(new Entry(entry.Description, Math.Max(border.Start, entry.Start), Math.Min(border.Finish, entry.Finish), -1));
						}
					}
		...
		public void Add(DataResponse response)
		{
			switch (response.ResponseType)
			{
				...
				case DataResponse.Type.EventFrame:
					{
						int id = (int)Utils.ReadVlqUInt(response.Reader);
