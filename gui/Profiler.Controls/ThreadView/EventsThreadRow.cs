...
namespace Profiler.Controls
{
	public class EventsThreadRow : ThreadRow
	{
		...
		void BuildMeshNode(DirectX.DynamicMesh builder, Durable unitTimeSlice, EventNode node, int level, Color parentColor)
		{
			...
			if (node.Description.Color.A == 0 &&
				Profiler.Data.Utils.EqualColors(nodeColor, parentColor))//autogenerated color, but looks ugly if same, change ForceColor
			{
				nodeColor = Color.FromRgb(nodeColor.B, nodeColor.R, nodeColor.G);//just swizzle colors
				if (Profiler.Data.Utils.EqualColors(nodeColor, parentColor))
					nodeColor = Color.FromRgb(nodeColor.B, nodeColor.R, nodeColor.G);//swizzle again
			}
